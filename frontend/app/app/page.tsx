...
export default function WorkspaceApp() {
  ...
  useEffect(() => {
    ...
    // Attempt to seed from local cached user if available
    ...
    // Verify token validity with /auth/me
    getCurrentUser()
      .then((user) => {
        setCurrentUser(user);
        setIsAuthChecking(false);
    ...
    window.addEventListener("devmind:auth-unauthorized", handleUnauthorized);

    return () => {
      window.removeEventListener("devmind:auth-unauthorized", handleUnauthorized);
    };
  }, [router]);
  ...
  }, [isAuthChecking, refreshConversationsList]);
  ...
  if (isAuthChecking) {
    return (
      <div className="flex h-screen w-full items-center justify-center bg-[#111111] text-[#e2e2e2]">
        <div className="flex flex-col items-center gap-3">
          <Loader2 className="w-8 h-8 text-[#3B82F6] animate-spin" />
          <p className="text-xs text-[#8c909f] font-mono">Verifying authentication session...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex h-screen w-full overflow-hidden bg-[#111111] text-[#e2e2e2] font-sans antialiased">
      {/* Left Sidebar with Persistent Recent Chats & User Profile */}
      <Sidebar
